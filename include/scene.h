...
public:
	...
	CameraC(double fovy, glm::vec3 eye, glm::vec3 target, glm::vec3 up) {
		fovy = fovy;
		eye = eye;
		target = target;
		up = up;
...
public:
	...
	CameraC* _parse_camera_config_JSON() {
		...
		return new CameraC(fovy, eye, target, up);
	}
